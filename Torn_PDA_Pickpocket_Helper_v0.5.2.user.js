// ==UserScript==
// @name         Torn PDA Pickpocket Helper v0.5.2
// @namespace    torn-pda-local
// @version      0.5.2
// @description  Torn PDA-safe pickpocket detection with one-tap picking and automatic result closing
...
(function () {
    ...
    function onPickpocketPage() {
        ...
        if (document.querySelector('.pickpocketing-root, [class*="pickpocketing"]')) {
            return true;
        }

        // Torn PDA can keep the address/title at "Crimes" while displaying
        // Pickpocketing. Confirm the visible page from its heading and targets.
        const pageText = String(document.body?.innerText || '');
        const hasHeading = /(^|\n)\s*PICKPOCKETING\s*(\n|$)/i.test(pageText);
        const hasTarget = ALL_TARGETS.some(name => pageText.includes(name));

        return hasHeading && hasTarget;
    }
    ...
    function updateScanTimer() {
        ...
        if (getScanEnabled()) scanTimer = setInterval(scan, SCAN_INTERVAL_MS);
        updateScanControl();
    }
